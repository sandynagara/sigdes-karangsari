...
function Sidebar({ queryNama, bangunan, open }) {
    ...
    >
      ...
      <div className="sidebar-info">
        ...
        <div className="sidebar-bangunan">
          ...
          <div style={{ margin: "10px" }}>Luas : {data.luas} m^2</div>
          <div style={{ margin: "10px" }}>Dusun : {data.nama_dusun}</div>
          <div style={{ margin: "10px" }}>{data.rt}</div>
        </div>
      
        
        <div className="sidebar-posisi"></div>
      </div> </div>: <div style={{height:"100%",display:"flex",alignItems:"center",justifyContent:"center"}}>No one feature selected</div> }
    </div>
  );
}
